...
class DataModel extends Database {
    ...
    public function update($nim, $nama, $email, $status) {
        $stmt = $this->conn->prepare("UPDATE alumni SET nama_lengkap=?, email=?, status_alumni=? WHERE nim=?");
        $stmt->bind_param("ssss", $nama, $email, $status, $nim);
        return $stmt->execute();
    }
